...
class Grid:
    ...
    def turn_into_grid(self, items, max_length, max_height):
        ...
        item_height = self.get_item_dimension(columns, max_height, self.height_buffer)
        item_length = self.get_item_dimension(rows, max_length, self.length_buffer)
        ...
        for x in range(len(items)):
            ...
            row_number = floor(x / rows)

            left_edge = base_left_edge + self.get_dimension_change(columns, item_length, self.length_buffer)
            top_edge = base_top_edge + self.get_item_dimension(rows, item_height, self.height_buffer)
    ...
    def get_item_dimension(self, grid_dimension, item_dimension_max, buffer_between_items):
        remaining_dimension = buffer_between_items * (grid_dimension - 2)
